...
describe('plannerStore', () => {
  ...
  describe('moveRecipe', () => {
    it('removes from source and adds to target', async () => {
      setState({
        plans: {
          '2025-06-02': { id: 'p1', date: '2025-06-02', meals: { ...emptyMeals(), lunch: ['r1'] } },
          '2025-06-03': { id: 'p2', date: '2025-06-03', meals: emptyMeals() },
        },
        loading: false,
      });

      const removedSource = planRow('p1', '2025-06-02', emptyMeals());
      const addedTarget = planRow('p2', '2025-06-03', { ...emptyMeals(), dinner: ['r1'] });

      let callCount = 0;
      mockFrom.mockImplementation(() => {
        callCount++;
        if (callCount === 1) {
          return createQueryMock({ data: removedSource, error: null });
        }
        return createQueryMock({ data: addedTarget, error: null });
      });

      await getState().moveRecipe('2025-06-02', 'lunch', '2025-06-03', 'dinner', 'r1');

      expect(getState().plans['2025-06-02'].meals.lunch).toEqual([]);
      expect(getState().plans['2025-06-03'].meals.dinner).toEqual(['r1']);
    });

    it('rolls back source on target failure', async () => {
      setState({
        plans: {
          '2025-06-02': { id: 'p1', date: '2025-06-02', meals: { ...emptyMeals(), lunch: ['r1'] } },
          '2025-06-03': { id: 'p2', date: '2025-06-03', meals: emptyMeals() },
      ...
      mockFrom.mockImplementation(() => {
        ...
        if (callCount === 2) {
          return createQueryMock({ data: null, error: { message: 'Upsert failed' } });
        }
        return createQueryMock({ data: null, error: null });
      });

      await expect(
        getState().moveRecipe('2025-06-02', 'lunch', '2025-06-03', 'dinner', 'r1'),
      ).rejects.toThrow('Failed to move recipe');
    ...
    it('throws when source plan does not exist', async () => {
      await expect(
        getState().moveRecipe('2025-06-02', 'lunch', '2025-06-03', 'dinner', 'r1'),
      ).rejects.toThrow('No meal plan found for the source date');
    });
  });
});
